...
def get_rains(code):
    arr = np.load("/home/nestormann/Documents/PPS/DeepWeather/rain.npy")
    idx = airports.index(code)
    return arr[:, idx].astype(np.int32)

def get_era_full(param, level):
    arr = np.load("/home/nestormann/Documents/PPS/DeepWeather/{}{}_uint8.npy".format(param, level))
...
def train_model(airport):
    ...
    X = np.zeros((13141, 80, 120, 8, 3),dtype=np.float16)
    ...
    model.fit(X, b, batch_size=20, epochs=1, verbose=1, validation_split=0.2, callbacks=[csv_logger])
